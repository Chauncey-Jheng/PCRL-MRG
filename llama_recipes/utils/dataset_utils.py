...
def get_custom_dataset(train_config, dataset_config, tokenizer, split: str):
...
def get_preprocessed_dataset(
    tokenizer, train_config, dataset_config, split: str = "train"
) -> torch.utils.data.Dataset:
    if not dataset_config.dataset in DATASET_PREPROC:
        raise NotImplementedError(f"{dataset_config.dataset} is not (yet) implemented")

    def get_split():
        if not split in ("train", "test", "validation", "valid", "val"):
            raise ValueError(f"'{split}' is not supported for split argument")
        if split == "train":
