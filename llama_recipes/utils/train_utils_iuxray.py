...
import json
from llama_recipes.pycocoevalcap.metrics_for_eng_mrg import compute_scores
...
def train_using_conditional_gen_eval(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, fsdp_config=None, local_rank=None, rank=None, wandb_run=None):
    """
    Trains the model on the given dataloader
    ...
    Args:
        ...
        gradient_accumulation_steps: The number of steps to accumulate gradients before performing a backward/update operation
        num_epochs: The number of epochs to train for
        local_rank: The rank of the current node in a dist23ributed setting
        train_config: The training configuration
        eval_dataloader: The dataloader containing the eval data
        tokenizer: tokenizer used in the eval for decoding the predicitons
    ...
    if train_config.save_metrics:
        metrics_filename = f"{train_config.output_dir}/metrics_scores-{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.json"
        train_step_perplexity = []
        train_step_loss = []
        scores_list = []
    ...
    for epoch in range(train_config.num_epochs):
        ...
        if train_config.run_validation:
            scores = evaluation_conditional_gen(model, train_config, eval_dataloader, local_rank, tokenizer, wandb_run)
            if train_config.save_metrics:
                scores_list.append("epoch " + str(epoch) + ":" + str(scores))
            eval_epoch_metric = scores["BLEU_4"]

            checkpoint_start_time = time.perf_counter()
            # if train_config.save_model and eval_epoch_metric > best_val_metric:
            ...
            checkpoint_end_time = time.perf_counter() - checkpoint_start_time
            checkpoint_times.append(checkpoint_end_time)
            if eval_epoch_metric > best_val_metric:
                best_val_metric = eval_epoch_metric
                if train_config.enable_fsdp:
                    if rank==0:
                        print(f"best eval metric on epoch {epoch+1} is {best_val_metric}")
                else:
                    print(f"best eval metric on epoch {epoch+1} is {best_val_metric}")
        ...
        if train_config.save_metrics:
            metrics_data = {
                ...
                "train_epoch_perplexity": train_prep,
                "NLG_scores": scores_list,
            }
...
def evaluation_conditional_gen(model,train_config, eval_dataloader, local_rank, tokenizer, wandb_run):
    model.eval()
    sample_ids = []
    eval_preds = []
    ground_truth = []
    ...
    with MemoryTrace() as memtrace:
        for step, batch in enumerate(tqdm(eval_dataloader,colour="green", desc="Eval Epoch", dynamic_ncols=True)):
            ...
            sample_ids.append(
                tokenizer.batch_decode(sample_id, skip_special_tokens=True)
            )
    ...
    for i in range(len(eval_preds)):
        for j in range(len(eval_preds[i])):
            if len(quiz_ids) != 0:
                re_gt_sample = {
                "quiz_id": sample_ids[i][j],
                "sample_id": sample_ids[i][j],
                "prediction": eval_preds[i][j].replace('\n', ' ').replace('\r', ' '),
                "ground_truth": ground_truth[i][j].replace('\n', ' ').replace('\r', ' '),
                }
            else:
                re_gt_sample = {
                    "sample_id": sample_ids[i][j],
    ...
    save_file = f"{train_config.output_dir}/Eval_result-{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.json"
